...
- PostgreSQL 16 with PostGIS extension stores incidents, businesses, audit logs, and row-level security policies.
- MinIO (S3-compatible) buckets store raw email blobs and attachments in private storage with hashed object keys.
