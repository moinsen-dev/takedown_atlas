...
export function ModerationQueue({ initialQueue }: { initialQueue: ModerationQueueEntry[] }) {
  const [queue] = useState(initialQueue);

  return (
    <div className="space-y-4">
      {queue.map((item) => (
        <article key={item.submission.id} className="rounded-2xl border border-brand-slate/10 bg-white/80 p-6 shadow-sm">
          <header className="flex flex-wrap items-start justify-between gap-3">
            <div>
              <p className="text-sm font-semibold text-brand-dark">Submission {item.submission.id.slice(0, 8)}</p>
              <p className="text-xs text-brand-slate/70">Received {new Date(item.queue_received_at).toLocaleString()}</p>
            </div>
          ...
          <p className="mt-4 text-sm text-brand-slate/80">
            {item.submission.ai_summary ?? 'Awaiting AI-assisted summary. Review headers and redact PII.'}
          </p>
      ...
      {queue.length === 0 && (
        <p className="rounded-2xl border border-dashed border-brand-slate/20 bg-white/60 px-4 py-6 text-center text-sm text-brand-slate/70">
          Moderation queue is clear. New submissions will appear here once processed.
        </p>
      )}
    </div>
  );
